...
const updateOneBox = (
  ...
  boxUpdatedValues: BoxPropertiesOptional
) => {
...
const updateColumnsVerticalIndexes = (
  ...
  const lockLastPlayedNr = updateOneBox(
    state,
    columnVerticalIndex,
    rowHorizontalIndex,
    { NrLocked: true }
  ...
  return {
    ...state,
    containerOfRows: lockLastPlayedNr,
    columnsVerticalIndexes: state.columnsVerticalIndexes.map(
      (columnVertical_Index, index) =>
...
const updateColumIndexOnKeyCPress = (
  state: InitialStateType,
  columnVerticalIndex: number,
  rowHorizontalIndex: number
) => {
  ...
  const lastNumberOnPlay =
    state.containerOfRows[columnVerticalIndex][rowHorizontalIndex];

  if (lastNumberOnPlay.value === 0) {
    return state;
  } else {
    const updatedVersion = updateColumnsVerticalIndexes(
      state,
      rowHorizontalIndex,
      columnVerticalIndex
    );
    const verticalIndex =
      updatedVersion.columnsVerticalIndexes[rowHorizontalIndex];
    ...
    if (!updatedVersion.containerOfRows[verticalIndex]) {
      ...
      return {
        ...updatedVersion,
        containerOfRows: [...updatedVersion.containerOfRows, RowConstructor()],
      };
    } else {
...
const updateBox = (
  ...
  const updateNumberInsideOfARow = updateOneBox(
    state,
    columnVerticalIndex,
    rowHorizontalIndex,
    { AmountTimesAdded: 1 }
  ...
  const checkIf3Times =
    updateNumberInsideOfARow[columnVerticalIndex][rowHorizontalIndex]
      .AmountTimesAdded === 3
      ? true
      : false;
  const columHasBeenFilled =
    updateNumberInsideOfARow[updateNumberInsideOfARow.length - 1][
      rowHorizontalIndex
    ].value === 0
      ? false
      : true;
  const checkIfLockedNr =
    updateNumberInsideOfARow[columnVerticalIndex][rowHorizontalIndex].NrLocked;
  if (checkIfLockedNr) {
    return { updatedState: { ...state } };
  }
  ...
  if (checkIf3Times) {
    ...
    const updatedStateWithlockNumber = {
      ...updateNr,
      containerOfRows: updateOneBox(
        updateNr,
        columnVerticalIndex,
        rowHorizontalIndex,
        { NrLocked: true }
...
const checkColumnStatus = (
  ...
  const { checkIf3Times, columHasBeenFilled, updatedState } = updateBox(
    state,
    columnVerticalIndex,
    rowHorizontalIndex
  );
